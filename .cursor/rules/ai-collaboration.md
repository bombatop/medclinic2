...
- **Commit messages**:
  - Use **lowercase** for the subject and body (no leading capital or title case).
  - Do **not** add phrases like "made by Cursor", "generated by AI", or similar tool-attribution text.
  - Describe the change plainly (e.g. `add rule for commit message style`, `fix login redirect`).
  - Do not use --trailer command for commits
  - Do not include .plan.md files in commits
